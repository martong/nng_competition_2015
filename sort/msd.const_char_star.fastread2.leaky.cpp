...
int main() {
    ...
    int totalSize = 0;
    do {
        bytesInBuffer = std::fread(&array[totalSize], 1, arraySize - totalSize,
                stdin);
        totalSize += bytesInBuffer;
    ...
    for (int i = 0; i < totalSize; ++i) {
        if (array[i] == '\n') {
            vector[stringIndex].length = i - currentPosition;
            vector[stringIndex].data = &array[currentPosition];
            currentPosition = ++i;
            ++stringIndex;
        }
    }
