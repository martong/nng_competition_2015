...
void sort(Strings& a, int lo, int hi, int d, Strings& aux) {
    ...
    if (d == 0) {
        ...
        for (int r = 0; r < R; r++) {
            ...
            Future f = std::async(std::launch::deferred, [&a, &count, lo, r, d, new_aux = std::move(new_aux)]() mutable {
        ...
        Threads threads;
        // Core™ i5-4200U Processor has 2 cores, 4 threads
        const int NUM_THREADS = 8;
        for (int i = 0; i < NUM_THREADS; ++i) {
            std::thread t([&index, &futures](){
                while (index < (int)futures.size() - 1) {
                    futures[index.fetch_add(1)].get();
                }
            });
