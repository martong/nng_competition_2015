#ifndef TETRIS_POINTRANGE_HPP
#define TETRIS_POINTRANGE_HPP
...
public:
    ...
    {
        if (begin_ == end_) {
            throw std::out_of_range("front() cannot be called on empty PointRange.");
        }
        return begin_;
    }
    Point back() const
    {
        if (begin_ == end_) {
            throw std::out_of_range("back() cannot be called on empty PointRange.");
        }
